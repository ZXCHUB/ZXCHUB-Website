...
export default function Products() {
  ...
  const filteredProducts = useMemo(() => products.filter(product => {
    ...
    if (mode === 'free') return startingPrice === 0;
    if (mode === 'premium') return startingPrice > 0;
    return true;
        ...
        <section className="relative mb-12 overflow-hidden border border-white/10 bg-[#09090d] p-6 sm:p-10">
          ...
          <div className="relative">
            ...
            <p className="mt-4 max-w-2xl text-zinc-400">
              Browse high-performance Roblox scripts developed for ZXCHUB. Pick a script, choose a duration, and receive your key after checkout.
            </p>
          </div>
        </section>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filteredProducts.map(script => {
              ...
              const startingPrice = script.variants?.length
                ? Math.min(...script.variants.map((variant: any) => Number(variant.price || 0)))
                : 0;
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    ...
                    <p className="mt-2 line-clamp-3 min-h-[4.5rem] text-sm leading-6 text-zinc-400">
                      {script.description || 'Premium ZXCHUB script with instant key delivery after purchase.'}
                    </p>
                    ...
                    <div className="mt-5 flex items-center gap-5 border-t border-white/10 pt-4 text-xs text-zinc-500">
                      ...
                      <span className="ml-auto font-mono text-zinc-400">{stock > 0 ? `${stock} keys` : 'No stock'}</span>
                    </div>

                    <div className="mt-5 flex items-end justify-between">
                      <div>
                        <div className="text-[10px] font-black uppercase tracking-[0.18em] text-zinc-600">Starting at</div>
                        {profile?.discountPercentage ? (
                          <div className="flex items-center gap-2">
                            <span className="text-sm text-zinc-600 line-through">${startingPrice.toFixed(2)}</span>
                            <span className="text-2xl font-black text-white">${finalStartingPrice.toFixed(2)}</span>
                          </div>
                        ) : (
                          <div className="text-2xl font-black text-white">${startingPrice.toFixed(2)}</div>
                        )}
                      </div>
